...
to the CLI. No tool runs a command or writes a file on a machine
(`mcp_test.go` enforces it); that is `start_job`.
...
| `list_storage` | every cluster with its members and health, and every workspace with its id, path and the remotes it appears on | read-only |
...
| `cluster_create`, `cluster_add_member` | pool mounted paths into one filesystem on a gateway | destructive |
| `workspace_create`, `workspace_member` | share a directory on a cluster with named remotes; add or remove one | destructive |
| `propose` | one redacted issue on the project's repository | open-world |
